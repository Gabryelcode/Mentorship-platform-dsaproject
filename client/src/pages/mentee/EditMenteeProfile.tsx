...
const EditMenteeProfile = () => {
  ...
  useEffect(() => {
    ...
    axios.get(`/auth/profile/${user.id}`).then(res => {
      setFormData({
        name: res.data.name || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      navigate(user.role === 'mentor' ? '/dashboard' : '/mentee/dashboard'); // Adjust if mentees use a different route
    } catch (err: any) {
  ...
  return (
    <div className="p-6 max-w-xl mx-auto bg-white shadow-md rounded">
      <h1 className="text-2xl font-bold text-green-700 mb-4">Edit Mentee Profile</h1>
